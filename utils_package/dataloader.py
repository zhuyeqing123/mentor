...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        self.eval_u = torch.tensor(self.eval_u, dtype=torch.float32).to(self.device)
    ...
    def _get_pos_items_per_u(self, eval_users):
        ...
        u_ids_tensor = torch.tensor(u_ids, dtype=torch.long)
        i_ids_tensor = torch.tensor(i_ids, dtype=torch.long)

        # 使用 torch.stack 合并这些张量
        return torch.stack([u_ids_tensor, i_ids_tensor])
